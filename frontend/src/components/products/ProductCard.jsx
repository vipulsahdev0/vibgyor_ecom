...
export default function ProductCard({ product }) {
  ...
  const finalPrice    = Number(product.finalPrice ?? product.discountedPrice ?? product.price ?? 0);
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5">
      ...
      <div className="relative overflow-hidden bg-slate-50">
        ...
        {/* Stock badge */}
        <span className={`absolute left-3 top-3 rounded-full px-2.5 py-1 text-[11px] font-semibold ${
          outOfStock ? "bg-rose-500 text-white" : "bg-emerald-500 text-white"
        }`}>
        ...
        <div className="absolute right-3 top-3 flex flex-col gap-2 opacity-0 transition-all duration-200 group-hover:opacity-100">
          ...
          <button
            ...
            className={`flex h-9 w-9 items-center justify-center rounded-full shadow-md transition-all duration-150 disabled:opacity-50 ${
              inWishlist
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-semibold text-white transition-all duration-150 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 ${
              cartAdded
